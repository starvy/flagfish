...
func (s *Server) adminOpsError(ctx context.Context, err error, action string) error {
	...
	case errors.Is(err, adminops.ErrFieldNotFound):
		return huma.Error404NotFound("field not found")
	case errors.Is(err, adminops.ErrPageNotFound):
		return huma.Error404NotFound("page not found")
	case errors.Is(err, adminops.ErrPageRouteTaken):
		return huma.Error409Conflict("that page route is already taken")
